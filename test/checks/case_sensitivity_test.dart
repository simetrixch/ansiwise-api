...
/// through the filesystem, and the Windows filesystem opens the wrong case without complaint. For
/// the same reason a check that asked whether the file opens would be green here and prove nothing.
/// The comparison is against the DIRECTORY LISTING instead: [SourceTree.on] walks the tree by
/// listing it, so its paths carry each name as the disk spells it, and every directive's resolved
/// path is compared to them byte for byte.
///
...
void main() {
  ...
  group('counter-probe', () {
    ...
    test('an import of a file that is on disk under no spelling is not a finding here', () {
      final SourceTree planted = SourceTree.planted(<String, String>{
        'lib/broken.dart': "import 'steps/not_there.dart';",
      });
      expect(
        caseMismatchesIn(planted),
        isEmpty,
