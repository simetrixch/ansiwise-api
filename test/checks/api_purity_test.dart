...
///
/// So the scan is over every byte of the package — code, comment and fixture alike — and not over
...
void main() {
  ...
  test('this framework names no platform in ${scannedPaths.join(', ')}', () {
    expect(
      occurrencesOfForbiddenWords(tree, forbidden),
      isEmpty,
      reason:
          'each finding reads <file>:<line>:<text>, and the fix is to say what the framework does '
          'rather than which tool a plugin would use to do it',
    );
  });
  ...
  group('counter-probe', () {
      ...
      for (final String word in forbidden) {
        ...
        expect(
          ...
          reason:
              ...
              "'$word', which would forbid names this framework may legitimately use",
        );
      }

      final SourceTree captured = SourceTree.planted(<String, String>{
        'lib/innocent.dart': 'const String snapshotOfState = "a captured state";',
      });
      expect(
        occurrencesOfForbiddenWords(captured, forbidden),
        isEmpty,
        reason: 'a captured state is something this framework may speak of',
      );
    });
    ...
    test('a file under test/ is reported, so test/ has not fallen out of scope', () {
      final SourceTree planted = SourceTree.planted(<String, String>{
        'test/config/planted_test.dart': _everyWordIn(forbidden),
      });
      expect(occurrencesOfForbiddenWords(planted, forbidden), isNotEmpty);
    });
...
///
/// Named one at a time rather than as "the repository minus tools", so adding a directory to the
...
List<String> forbiddenWords(Directory root) {
  final File file = File(p.join(root.path, p.joinAll(wordListPath.split('/'))));
  if (!file.existsSync()) {
    throw StateError(
