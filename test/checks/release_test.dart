...
///
/// **What a release of THIS repository is.** Nothing is built here and nothing is downloaded: this
/// package is a library, and every package that depends on it names it as a git dependency and gets
/// whatever the `ref` it states names. So the tree at the tag IS what is released, and a release is
/// the existence of a tag somebody can name. That is why nothing below asks about an attached file,
/// and why the one line that matters on the release page is the `ref` a dependent writes.
///
...
/// list of argument lists in order, and both claims are readable in it — the screen RUNS ONLY READS,
/// and a version and a channel that are typed reach `git push` as the last of exactly eight
/// commands. The refusing half needs none of that and is driven as the real program, on this
...
void main() {
  group('which tags start a release', () {
    ...
    test('is three numbers, a channel, and fourteen digits — and nothing else', () {
      ...
      for (final String notATag in <String>[
        ...
        expect(
          ...
          isFalse,
          reason:
              '"$notATag" is no tag under this grammar, and a tag that starts a release is what a '
              'dependent then names as the `ref` it resolves',
        );
      }
    });
    ...
    test('COUNTER-PROBE: a filter that ends in a star admits what this grammar refuses', () {
      // PLANTED: three numbers, and anything at all after a hyphen. They admit a tag with no channel
      // and a tag with no stamp, which are the two things the release page and the ceiling read.
      final TagFilter loose = TagFilter.ofWorkflow(
        _workflowTriggeringOn(<String>["'[0-9]+.[0-9]+.[0-9]+'", "'[0-9]+.[0-9]+.[0-9]+-*'"]),
      );
      expect(loose.accepts('0.1.0'), isTrue);
      expect(loose.accepts('0.1.0-beta.2'), isTrue);
      expect(loose.accepts('0.1.0-'), isTrue);
      expect(
        loose.accepts('0.1.0-stable-20260821194500'),
        isTrue,
        reason: 'the innocent case, or nothing means anything',
      );
  ...
  group('the order the screen lists releases in', () {
    ...
    test('the version pubspec declares is read from the manifest, and written back into it', () {
      expect(declaredVersionIn('name: ansiwise_core\nversion: 1.4.2\n\ndependencies:\n'), '1.4.2');
      expect(declaredVersionIn('name: ansiwise_core\ndependencies:\n  yaml: ^3.1.3\n'), isNull);
      expect(
        pubspecWithVersion('name: ansiwise_core\nversion: 1.4.2\n\ndependencies:\n', '1.5.0'),
        'name: ansiwise_core\nversion: 1.5.0\n\ndependencies:\n',
      );
      expect(
        pubspecWithVersion('name: ansiwise_core\ndependencies:\n', '1.5.0'),
  ...
  group('a version and a channel that are typed', () {
    test('bump, commit, annotated tag, HEAD, tag — in that order and no other', () async {
      ...
      expect(
        outcome.text,
        contains('NOTHING IS ATTACHED TO IT'),
        reason:
            'a release of this repository carries no file, and a person expecting one would go '
            'looking on the release page for something nobody put there',
      );
      expect(
        outcome.text,
        contains('ref: $tag'),
        reason:
            'the tree at the tag is the whole of what is released, so the line that names it is the '
            'only thing this release hands anybody — and it is spelled out here rather than asked '
            'of thePin, because a check that composed the line the same way the program does would '
            'agree with the program whatever either of them said',
      );
      expect(
        outcome.text,
      ...
      expect(
        outcome.text,
        contains('not something this run did'),
        reason:
            'a release is not a resolution: nothing that depends on this package moves onto the new '
            'tag until an edit in its own pubspec.yaml says so',
      );
    });
    ...
    test(
      ...
      () async {
        ...
        // this filter does not, which is the opposite direction to the gamma probe above — and the
        // two refusals have to be told apart, or a person is sent to the wrong file.
        final ScriptedGit git = _gitAnswering(<String, GitAnswer>{});
    ...
    test('COUNTER-PROBE: a dirty working tree is refused before the manifest is touched', () async {
      final ScriptedGit git = _gitAnswering(<String, GitAnswer>{
        'status --porcelain': const GitAnswer(
          status: 0,
          output: ' M lib/ansiwise_core.dart\n?? scratch\n',
        ),
      });
      final FakeManifest manifest = _aManifestAt('0.1.0');
      ...
      expect(
        outcome.text,
        contains('M lib/ansiwise_core.dart'),
  ...
  group('the notes the workflow publishes', () {
    test('name the channel, the line that pins this tag, and every commit since the release '
        'before', () async {
      ...
      expect(
        ...
        reason:
            'the page is the whole of what this release hands anybody, so it has to carry the line '
            'that names the tag rather than leave it to be worked out',
      );
  ...
  group('what the workflow does with the tag', () {
    test('runs this repository\'s own gate before anything is published', () {
      final String workflow = File(releaseWorkflowPath).readAsStringSync();

      expect(
        workflow,
        contains('dart run tool/ci.dart'),
        reason:
            'a release of a library that does not pass its own checks is worse than none: nothing '
            'is built here, so the tree at the tag is what somebody else resolves and the finding '
            'arrives in their repository',
      );
      expect(
        workflow,
        contains('needs: gate'),
        reason:
            'the gate standing beside the publish rather than in front of it would prove '
            'nothing about what was published',
      );
      expect(
        workflow,
        contains(r'dart run tool/release_notes.dart "$TAG" release-notes.md'),
        reason:
            'the page and the pre-release marking are read out of the tag by that program, and a '
            'second reading written into a `run:` block is one no check can reach',
      );
    });

    test('publishes a release with no file attached to it', () {
      final String workflow = File(releaseWorkflowPath).readAsStringSync();

      expect(workflow, contains('gh release create "$_tagPlaceholder"'));
      for (final String never in <String>['upload-artifact', 'download-artifact', 'dart compile']) {
        expect(
          workflow,
          isNot(contains(never)),
          reason:
              'this package is compiled into nothing and the tree at the tag is what is released — '
              'a job building or attaching a file here would publish something nobody resolves',
        );
      }
    });
  });
...
/// How the publish step names the tag it was started by, as the workflow's `env` spells it.
const String _tagPlaceholder = r'$TAG';
...
FakeManifest _aManifestAt(String version) =>
    FakeManifest('pubspec.yaml', 'name: ansiwise_core\nversion: $version\n\ndependencies:\n');
