...
/// until its dependencies are there — and every remaining package must still run, because one
/// failure hiding the rest is how the next run finds a second problem that was there all along.
void main() {
  test('a green run says exactly what the gate is read by', () async {
    ...
    expect(
      verdict.line,
      'ci: OK — every check green',
      reason: 'this line is the done-criterion of every piece of work in this repository',
    );
  });
  ...
  test('a package whose resolution failed is named, and not tested', () async {
  ...
  test('a red analysis does not stop the suites from running', () async {
    final FakeDartToolchain toolchain = FakeDartToolchain()..streamedExitCode = 1;
    await _gate(toolchain, <String>['one', 'two']).run();
    expect(
      toolchain.calls.where((ToolCall call) => call.what == 'test'),
      hasLength(2),
      reason: 'one failure hiding the rest is how the next run finds a second problem',
    );
  });

  test(
    'the analysis program is started once, from the repository and not from a package',
    () async {
      final FakeDartToolchain toolchain = FakeDartToolchain();
      await _gate(toolchain, <String>['one', 'two']).run();
      expect(
        toolchain.calls
            .where((ToolCall call) => call.what == 'run tool/analysis.dart')
            .single
            .directory,
        '/work',
      );
  ...
  test('a tree with no Dart package in it is red, not quietly green', () async {
    final GateVerdict verdict = await PackageGate(
      toolchain: FakeDartToolchain(),
      packages: const <DartPackage>[],
      log: CollectedGateLog(),
      analysisRoot: '/work',
    ).run();
    expect(
...
/// One scratch package per name, in that order, each carrying the test/ directory that makes a
/// suite run.
...
Directory _scratch() {
  final Directory directory = Directory.systemTemp.createTempSync('ansiwise-package-gate-');
