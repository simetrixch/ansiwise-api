...
void main() {
  ...
  group('what the client is told about such a row', () {
    test('the description names the measurement and the row, and no value', () {
      final ResolvedProgram program = ProgramResolver(
        registry(contentDefault: 'nft'),
      ).resolve(measureThenWrite());
      final ApiResponse answer = ProgramsEndpoint(_OneProgram(program)).one(const ProgramName('p'));

      final Map<String, Object?> described = (answer as Answered).payload as Map<String, Object?>;
      final List<Object?> steps = described['steps'] as List<Object?>? ?? const <Object?>[];
      final Map<String, Object?> row = steps.whereType<Map<String, Object?>>().last;
      final List<Object?> arguments = row['arguments'] as List<Object?>? ?? const <Object?>[];
      final Map<String, Object?> content = arguments.whereType<Map<String, Object?>>().firstWhere(
        (Map<String, Object?> each) => each['name'] == 'content',
      );

      expect(content['value'], isNull, reason: 'the default is not what this row will run with');
      expect(content['measured'], <String, Object?>{
        'name': 'host.backend',
        'produced_by': 'measures',
        'position': 0,
      });
    });
  });
...
/// A catalogue holding one program, so the endpoint can be called with it.
final class _OneProgram implements Catalogue {
  const _OneProgram(this._program);

  final ResolvedProgram _program;

  @override
  List<ResolvedProgram> get programs => <ResolvedProgram>[_program];

  @override
  ResolvedProgram? byName(ProgramName name) => _program.declared.name == name ? _program : null;
}
